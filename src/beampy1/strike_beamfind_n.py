# -*- coding: utf-8 -*-
"""Strike_beamfind_n.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12S3t1cH09b__ZiywDm_Oml0SX5xa2NmI
"""
...
class Strike_ImageTools :
    ...
    @staticmethod
    def np_gsk2(size, m, S, gain=1, rotation=0):
        def gaussian(xy, m, S, G):
            xy -= np.array(m)
            IS = np.linalg.inv(S)
            return np.exp( -np.dot(np.dot(xy,IS),xy)/2 ) * G
        sizex, sizey = size
        img = np.zeros([sizex,sizey],dtype=np.float)
        if np.shape(S) == (2,):
            S = np.array([[S[0],0],[0,S[1]]],dtype=np.float)
        if (rotation > 0):
            sr = np.sin(float(rotation))
            cr = np.cos(float(rotation))            
            S  = np.dot( S, np.array([[cr,-sr],[sr,cr]],dtype=np.float))
        for x in range(0,size[0]):
            for y in range(0,size[1]):
                img[x,y] = gaussian([x,y],m,S,gain)
        return img
...
img = Strike_ImageTools.np_gsk2([200,200],[100,100],[10,100],rotation=1)
...
number_of_gaussians = 2          #@param {type:"slider", min:1, max:15, step:1 }
dispersion_sigma_attenuation = 29 #@param {type:"slider", min:4, max:10, step:0.1}
gauss_shape_mean = 12            #@param {type:"number"}
gauss_shape_var  = 2             #@param {type:"number"}




def generator():  # number of gaussians
    def find_squared_grid_pos(id):
        # TODO add a grid of displacements to order the labels.....
        size = number_of_gaussians
        return [0, id * IMAGE_SIZE[1]/size - IMAGE_SIZE[1]/4]
    def gen_gaussian(xy):
        xy  = np.random.normal(xy,IMAGE_SIZE/dispersion_sigma_attenuation,2) 
        s   = np.random.normal(gauss_shape_mean, gauss_shape_var, 1)
        img = Strike_ImageTools.np_gsk(IMAGE_SIZE,xy,s)
        return np.append(xy,s), img
    import itertools    
    for i in itertools.count(1):
        xy_init = find_squared_grid_pos(0)
        xys, img = gen_gaussian(xy_init)
        for j in range(1,number_of_gaussians):
            xy_init = find_squared_grid_pos(j)
            xys_j, img_j = gen_gaussian(xy_init)
            xys = np.append(xys,xys_j)
            img += img_j
        yield ((i,xys), img)
...
ds = tf.data.Dataset.from_generator(
                ...
                output_shapes = (((),3 * number_of_gaussians), tuple(IMAGE_SIZE))
                )



#@title Parameters
learning_rate = 0.0007 #@param {type:"slider", min:0, max:0.002, step:0.0001}
num_steps = 500   #@param {type:"slider", min:0, max:1000, step:50}
batch_size = 16  #@param ["16", "36", "64", "100"] {type:"raw"}
display_step = 50 #@param {type:"integer"}
GAIN = 2          #@param {type:"integer"}

n_classes = 3 * number_of_gaussians
...
# Strike_ImageTools.show_images(X, xys).suptitle("TRAINING EXAMPLE")
# plt.show()
# exit()
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        # Convolution Layer 
        conv = tf.layers.conv2d(x, 64, 10, activation=tf.nn.relu, strides=2)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        # conv = tf.layers.max_pooling2d(conv, 2, 2)

        # Convolution Layer 
        conv = tf.layers.conv2d(conv, 64, 10, activation=tf.nn.relu, strides=2)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        # conv = tf.layers.max_pooling2d(conv, 2, 2)        
        ...
        fc = tf.layers.dense(fc, 1024 * number_of_gaussians )
        ...
        # Fully connected layer
        fc = tf.layers.dense(fc, 500 * number_of_gaussians )
        fc = tf.layers.dropout(fc, rate=dropout, training=is_training)        
